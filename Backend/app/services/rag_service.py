...
{hit.payload['page_content']}""")
    ...
    def build_prompt_with_context(self, input_dict: Dict[str, Any]) -> List[HumanMessage]:
        ...
        prompt = f"""
Сен Қазақстандағы ҰБТ (Бірыңғай ұлттық тестілеу) бойынша репетиторсын, оқушыларды ЕНТ-ға дайындауға маманданғансын.
Сенің мақсатың - тек берілген контекст негізінде жауап беру, сонымен қатар оқушыға берілген контекстке сүйене отырып, сұрақтар қойып материалды түсінуге көмектесу. 
...
3. Егер контекстте ақпарат болмаса, "Мәтінде бұл сұраққа жауап жоқ" деп айт
